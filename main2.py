# -*- coding: utf-8 -*-
# ==============================================================================
# 🔥 PROJECT RUST v2.1 - THE MASTER'S CONTROL EDITION 🔥
#    Now with a Strategy Editor. You are the master now.
...
def load_and_clean_data(file_path):
    # This function is now stable and does not need changes.
    records = []
    try:
...
def dynamic_brain(df, strategies):
    # This function is now stable and does not need changes.
    best_strategy, best_score, backtest_period = list(strategies.keys())[0], -1, 15
    ...
    with Live(Spinner("arc", text="[bold yellow]ब्रेन पिछले प्रदर्शन का विश्लेषण कर रहा है...[/bold yellow]"), console=console, transient=True) as live:
        ...
        for name, strategy in strategies.items():
            ...
            for i in range(len(df) - backtest_period, len(df)):
                ...
                if 'otc' in preds and preds['otc'] and (actual_open in preds['otc'] or actual_close in preds['otc']): score += 1
...
# ==============================================================================
# 3. रणनीति और बैकटेस्टिंग फंक्शन्स (अपडेटेड!)
# ==============================================================================
def load_strategies():
    try:
        with open(STRATEGIES_FILE, 'r') as f: return json.load(f)
    except (FileNotFoundError, json.JSONDecodeError):
        console.print(f"[bold red]चेतावनी: '{STRATEGIES_FILE}' नहीं मिली या खराब है। डिफ़ॉल्ट रणनीतियाँ लोड की जा रही हैं।[/bold red]")
        return {
...
def create_new_strategy(strategies):
    """नया! यह फंक्शन यूजर को अपनी रणनीति बनाने की शक्ति देता है।"""
    os.system('cls' if os.name == 'nt' else 'clear')
    console.print(Panel(Text("🛠️ नई रणनीति बनाएं (Strategy Editor) 🛠️", justify="center"), border_style="magenta"))
    
    # 1. नाम पूछें
    name = Prompt.ask("[bold]नई रणनीति का नाम दें[/bold]", default="My_Secret_Weapon")
    if name in strategies:
        console.print(f"[red]'{name}' नाम की रणनीति पहले से मौजूद है।[/red]"); time.sleep(2); return
    ...
    try:
        selected_features = [all_features[int(i)-1] for i in selected_indices]
    except (ValueError, IndexError):
        console.print("[red]गलत चयन।[/red]"); time.sleep(2); return
    
    if not selected_features:
        console.print("[red]कोई फीचर नहीं चुना गया।[/red]"); time.sleep(2); return
        
    # 4. मॉडल चुनें
    model = Prompt.ask("[bold]कौन-सा मॉडल इस्तेमाल करना है?[/bold]", choices=["RandomForest", "LogisticRegression"], default="RandomForest")

    # 5. रणनीति को सेव करें
    new_strategy = {"lookback": lookback, "features": selected_features, "model": model}
    strategies[name] = new_strategy
    save_strategies(strategies)
    
    console.print("\n[bold green]🎉 आपकी नई रणनीति '{name}' सफलतापूर्वक सेव हो गई है! 🎉[/bold green]")
    time.sleep(2)

def run_backtesting(df, strategies):
    # This function is now stable and does not need changes.
    os.system('cls' if os.name == 'nt' else 'clear')
    console.print(Panel(Text("🔬 बैकटेस्टिंग मॉड्यूल 🔬", justify="center"), title="रणनीति प्रदर्शन विश्लेषण", border_style="red"))
    try:
        ...
        if days_to_test < 10 or days_to_test > len(df) - 20:
            console.print(f"[red]अमान्य दिन। कृपया 10 से {len(df) - 20} के बीच चुनें।[/red]"); time.sleep(2); return
    except ValueError: console.print("[red]अमान्य इनपुट। केवल नंबर डालें।[/red]"); time.sleep(2); return
    
    report_table = Table(title=f"[bold]पिछले {days_to_test} दिनों का रिपोर्ट कार्ड[/bold]", border_style="green")
    report_table.add_column("रणनीति (Strategy)", style="cyan", no_wrap=True); report_table.add_column("सफलता (Hits)", style="magenta", justify="center")
    report_table.add_column("कुल प्रयास (Attempts)", style="white", justify="center"); report_table.add_column("सफलता दर (Success %)", style="bold green", justify="right")
    ...
    with Live(Spinner("dots", text="[bold green]पुराने डेटा पर रणनीतियों का परीक्षण किया जा रहा है...[/bold green]"), console=console) as live:
        for name, strategy in strategies.items():
            ...
            start_index, end_index = len(df) - days_to_test, len(df)
...
def display_main_menu(strategies):
    os.system('cls' if os.name == 'nt' else 'clear')
    banner = Text("🔥 PROJECT RUST v2.1 - THE MASTER'S CONTROL 🔥", justify="center", style="bold yellow on red")
    console.print(Panel(banner, border_style="red"))
    
    strat_panels = [Panel(f"[bold]{name}[/bold]\n[cyan]Lookback:[/cyan] {s['lookback']}\n[cyan]Model:[/cyan] {s['model']}", border_style="magenta") for name, s in strategies.items()]
...
def main():
    while True:
        ...
        main_menu_table.add_row("[green]6.[/green]", "नई रणनीति बनाएं (Strategy Editor)")
        main_menu_table.add_row("[red]0.[/red]", "बाहर निकलें (Exit)")
        console.print(main_menu_table)

        main_choice = Prompt.ask("[bold yellow]👉 अपना एक्शन चुनें[/bold yellow]", choices=["0", "1", "6"], default="1")

        if main_choice == '0':
            console.print("[bold magenta]अलविदा भाई! फिर मिलेंगे।[/bold magenta]"); break
        elif main_choice == '6':
            create_new_strategy(strategies)
            continue

        available_markets = sorted([f for f in os.listdir(DATA_DIR) if f.endswith('.txt')])
        if not available_markets:
            console.print(f"[bold red]चेतावनी: '{DATA_DIR}' फोल्डर खाली है।[/bold red]"); time.sleep(2); continue
        ...
        choice_str = Prompt.ask("[bold yellow]👉 मार्केट नंबर चुनें (0 to go back)[/bold yellow]", choices=[str(i) for i in range(len(available_markets) + 1)], default="0")
        ...
        if df is None or len(df) < MIN_RECORDS_FOR_ANALYSIS:
            console.print(f"[bold red]'{market_name}' में विश्लेषण के लिए पर्याप्त ({MIN_RECORDS_FOR_ANALYSIS}) रिकॉर्ड नहीं हैं।[/bold red]"); time.sleep(2); continue
...
def run_predictions_for_market(market_name, df_featured, strategies):
    while True:
        os.system('cls' if os.name == 'nt' else 'clear')
        console.print(Panel(Text(f"🔥 {market_name.replace('.txt','').upper()} - ANALYSIS DECK 🔥", justify="center"), title="Project Rust v2.1", border_style="yellow"))
